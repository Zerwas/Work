...
Equational unification is interesting for term rewriting systems \cite{TRS}. We will only consider boolean unification in more detail but there is also a lot of work on other equational theories like $AC$ \cite{TRS}. For more information on the complexity of boolean unification look at \cite{cmplxBU}.
...
\begin{definition}
	...
	\end{itemize}
\end{definition}
%TODO To see why this classification makes sense lets consider the set 
%...
...
\end{definition}
$\moreGeneral$ is a is a quasi order since it obviously is reflexive and transitive. To see why we only demand equality modulo $\approx_E$ on $X$ and not on all Variables like we did in syntactic unification we fist need to define $E$-$minimal\ E$-$complete\ sets$.
\begin{definition}
...
Lets consider an arbitrary $A$-unifier $\theta$ of $S$. The substitution $\theta$ has the form $\theta(x):=x_1+\dots+x_n$ where the $x_i's$ are either $a$ or a variable.
