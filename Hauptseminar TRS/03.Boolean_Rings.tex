...
\[B:=\left\lbrace 
	...
	\begin{enumerate}
		...
		\item[(2)]The substitution $\sigma':=\left\lbrace y\mapsto x+1,z\mapsto 1+z\right\rbrace $ obviously is a $B$-unifier. Let $\varphi$ be a mapping with $\varphi(x):=\top$ for all $x\in V$ and $\widehat{\varphi}$ its homeomorphic extension.
		\begin{align*}
		\widehat{\varphi}\sigma'(w)=\begin{cases}
		(\top+1)^\BTwo=\bot& \text{if } w=y\\
		(1+\top)^\BTwo=\bot& \text{if } w=z\\
		\top&  \text{otherwise}
		\end{cases}
		\end{align*}
