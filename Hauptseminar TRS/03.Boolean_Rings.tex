...
\[B:=\left\lbrace 
	...
	\begin{enumerate}
		...
		\begin{definition}
		A $B$-unifier $\sigma$ of a $B$-unification problem $S$ is a \textbf{reproductive $B$-unifier} iff $\tau(\sigma(x))\approx_B\tau(x)$ for every unifier $\tau$ of $S$ and every $x$.
		\end{definition}
		Note that for a normal mgu $\sigma$ of a $B$-unification problem $S$ it only has to hold that for every $B$-unifier $\tau$ of $S$ there has to exist a substitution $\theta$ such that $\theta(\sigma(x))\approx_B\tau(x)$ for every $x\in X$. 
		...
		This algorithm is not as intuitive as successive variable elimination but much more interesting. The idea is to turn an arbitrary $B$-unifier $\tau$ of $t\approxq_B0$ into an mgu (even a reproductive $B$-unifier). The substitution $\sigma$ defined by
		\begin{align*}
		\sigma(x):=\begin{cases}
