...
\[B:=\left\lbrace 
	...
	\begin{enumerate}
		...
		\item If $s\approxq_B t$ has a unifier then $s\approxq t$ has a solution in $\BTwo$.
		\end{enumerate}
		\end{lemma}
		%TODO Note that we did not use iff because not every unifier can be viewed as a solution in $\BTwo$.